...
Для работы по построению маршрута между двумя произвольными домами не удобно использовать массив `roads`. Удобнее использовать другую структуру данных.
...
Робот это функция, выбирающая направление движения, она возвращает объект вида:

```js
{"direction": "Alice's House", "memory": []}
```

т.е. направление в сторону дома Алисы, и пустую память.

Ключ `memory` - это копия маршрута посылок `mailRoute` без пройденных вершин. Используется только двумя продвинутыми роботами.

Тактики выбора направления движения:
...
В соответствии с тактикой выбора направления следующего хода роботы могут быть:
...
randomPick вынесена в вспомогательную функцию для наглядности, чтобы реализовать принцип полиморфизма и показать, что данный код можно использовать повторно в других ситуациях.

#### Робот с памятью

`routeRobot` - Робот с памятью `memory`, использует тактику выбора направления движения на основе заранее спланированного маршрута [`mailRoute`](#маршрут). 
...
Сложный робот, который на вход принимает свое местоположение, список посылок и пустой маршрутный лист.
Т.к. лист пустой, то берется первая посылка из списка, если местоположение данной посылки не совпадает с текущем местом робота, то ищется маршрут до нее. Если посылка и робот в одном месте, то ищется маршрут до места адресата. В любом из этих случаев найденный маршрут запоминается в маршрутный лист. И выбор направления движения происходит только на основе маршрутного листа. Доставленная посылка удаляется из листа.
...
Поиск маршрута вынесен в отдельную функцию [`findRoute`]. В книге данная функция использует собственные алгоритм обхода графа на основе классического BFS.

##### Классический BFS

Breadth First Search (BFS) - алгоритм обхода графа в ширину, с помощью очереди FIFO.

При BFS происходит "поджигание" стартовой вершины, огнь от которой равномерно распространятся вширь пока не поглотит все дерево графа.
...
Массивы возвращаются как результат функции `bfs` и на основании них можно строить маршрут от стартовой точи и до желаемой:
...
Вышеприведенный код огромен, он исполняется каждый раз при поиске нового маршрута. В книге приведен более компактный код.
...
В книжном варианте нет массивов `visited`, `distances` и `parents`, также нет классической очереди `queue`.

Все данные хранятся в массиве `work`. В нем находятся вершины, которые нужно обследовать следующими и список маршрутов которые ведут к ним. Данный массив так же выполняет роль очереди. Это упрощает код, но немного усложняет понимание логики.

### Движок

Функции-роботы, возвращают только одно направление движения для конкретного состояния. Но, нужен механизм, автоматизирующий процесс запуска робота до полного опустошения массива посылок. Данным механизмом является функция-движок `runRobot`:
...
function runRobot(state, robot, memory) {
  for (let turn = 0;; turn++) { // бесконечный цикл
    if (state.parcels.length == 0) { // если все посылки доставлены,
      console.log(`Done in ${turn} turns`); // то вывести количество ходов
      break; // выход
    }
    // присвоить результат от функции-робота переменной action
...
При запуске движка с тупым роботом принимается два параметра - состояние и функция-робот:

```js
runRobot(VillageState.random(), randomRobot);
```

Весь обход зависит от случайности, минимальное зафиксированное мной количество ходов было 19, максимальное 173.
...
Передается три аргумента: состояние, функция-робот и память:
...
Для сравнение производительности роботов можно использовать функцию:

```js
// функция принимает четыре аргумента: первый робот, его память, второй робот, его память
function compareRobot(robot1, memory1, robot2, memory2){
  let sum1 = 0; // суммарное количество ходов для первого робота
  let sum2 = 0; // суммарное количество ходов для второго робота
  const tasks = 100; // количество циклов обхода деревни
  for(let i=0; i < tasks; i++){ // цикл обхода деревни
      const state = VillageState.random(); // одинаковое состояние для обоих роботов
      sum1 += countSteps(state, robot1, memory1); // расчет суммарного количества ходов для первого робота
      sum2 += countSteps(state, robot2, memory2); // расчет суммарного количества ходов для второго робота
  } 
  console.log(`Average moves: robot1 ${sum1/100} robot2 ${sum2/100}`); // вывод результата в консоль
}
...
Умный робот `goalOrientedRobot` имеет два недостатка:

1. Взяв первую посылку из списка робот занимается только ей
    - Идет к ней пока не подберет
    - Идет к адресату если посылка подобрана
    - Подбирает и доставляет другие посылки, если только он лежат на маршруте
2. Рассчитывает расстояние только до ближайщей посылки, которую нужно подобрать
    - Не учитывается совокупное расстояние суммы шагов до посылки + шагов до адресата

Предположим имеем:

- робота на Почте
- посылку A в доме Гретты с адресатом в Хижину
- посылку B в доме Эммы с адресатом на Почту

![goal](img/07_02.png)

Необходимо совершить 18 шагов для доставки всех посылок.

Но, посылка В всего лишь в шаговой доступности от посылки А, логичнее ее было бы забрать, потратив один ход, но съекономив на возврате после доставки посылки А в Хижину. Тогда нужно будет сделать всего 10 ходов, т.е. на 8 меньше.
...
В данной функции к каждому маршруту `route` добавляется свойство `pickUp`, которое определяет, нужно ли забирать посылку.

Вложенная функция `score` меняет булево свойство `pickUp` в зависимости от того, нужно ли забирать посылку: 0.5 в случае если посылку надо забрать, 0 в случае если ее надо доставить. Выбор 0.5 у автора скорее всего был эмпирическим. При замене данного коэффициента на 1, наблюдается уменьшение количества ходов, но не всегда. Примерна каждый пятый результат увеличивает количество ходов. (Возможно стоит изучить)

Метод `reduce` возвращает маршрут с наибольшим значением `score`, он вызывается при каждом новом создании списка маршрутов `routes`:

```js
// Маршрут 1 - Шаг 1: Из Post Office (начальное положение))
[
  {route: ["Marketplace", "Shop", "Grete's House"], pickUp: true},  // Посылка A
  {route: ["Marketplace", "Town Hall", "Daria's House", "Ernie's House"], pickUp: true}  // Посылка B
]
score({route: ["Marketplace", "Shop", "Grete's House"], pickUp: true}) = 0.5 - 3 = -2.5
score({route: ["Marketplace", "Town Hall", "Daria's House", "Ernie's House"], pickUp: true}) = 0.5 - 4 = -3.5
reduce(-2.5, -3.5) // -2.5 > -3.5 → Идем к А
// Маршрут 2 - Шаг 4: Из Grete's House (после подбора посылки A)
[
  {route: ["Farm", "Marketplace", "Alice's House", "Cabin"], pickUp: false},  // Доставка A
  {route: ["Ernie's House"], pickUp: true}  // Подбор B
]
score({route: ["Farm", "Marketplace", "Alice's House", "Cabin"], pickUp: false}) = 0 - 4 = -4
score({route: ["Ernie's House"], pickUp: true}) = 0.5 - 1 = -0.5
reduce(-4, -0.5) // -4 < -0.5 → Идем к B
// Маршрут 3 - Шаг 5: Из Ernie's House (после подбора посылки B)
[
  {route: ["Daria's House", "Town Hall", "Marketplace", "Alice's House", "Cabin"], pickUp: false},  // Доставка A
  {route: ["Daria's House", "Town Hall", "Marketplace", "Post Office"], pickUp: false}  // Доставка B
]
score({route: ["Daria's House", "Town Hall", "Marketplace", "Alice's House", "Cabin"], pickUp: false}) = 0 - 5 = -5
score({route: ["Daria's House", "Town Hall", "Marketplace", "Post Office"], pickUp: false}) = 0 - 4 = -4
reduce(-5, -4) // -5 < -4 → Идем к B
// Маршрут 4 - Шаг 9: Из Post Office (после доставки посылки B)
[
  {route: ["Alice's House", "Cabin"], pickUp: false}  // Доставка A
]
// Единственный вариант → Выбирается автоматически -> Идем к А
```

#### Постоянная группа

Задача создать персистентный (не мутирующий) класс PGroup, на основе Group из задания 3 главы 6:

```js
class PGroup {
  // Приватное поле members - массив
  #members;
  // Конструктор
  ...
  // Статическое свойство, которое позволяет новый объект класс без new
...
Фишкой данного примера является конструкция `static empty = new PGroup([])`. Тут `static` позволяет получить доступ к свойству `empty` без создания экземпляра класса, т.е. выполнить код, который в свою очередь создаст новый экземпляр класса.