...
Роботом это функция, выбирающая направление двигаться, она возвращает объект вида:

```js
{"direction": "Alice's House", "memory": []}
```

т.е. направление в сторону дома Алисы, и пустой памятью.
...
- Различные алгоритмы: [BFS](https://en.wikipedia.org/wiki/Breadth-first_search) [DFS](https://en.wikipedia.org/wiki/Depth-first_search) [A*](https://en.wikipedia.org/wiki/A*_search_algorithm)
...
Вышеприведенный код огромен, который исполняется каждый раз при поиске нового маршрута. В книге приведен более компактный код.
...
Функции-роботы, возвращают только одно направление движения для конкретного состояния. Но, нужен механизм, автоматизирующий процесс запуска робота до полного опустошения массива посылок. Данным механизмом выступает функция-движок `runRobot`:
...
То нужно будет сделать 18 шагов доставить все посылки
...
Вложенная функция `score` меняет булево свойство `pickUp` в зависимости от того, нужно ли забирать посылку: 0.5 в случае если посылку надо забрать, 0 в случае если ее надо доставить. 
